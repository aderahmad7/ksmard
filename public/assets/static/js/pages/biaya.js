...
function hitungTotal(tabel, data1, data2, element) {
    document.querySelectorAll(`${tabel}`).forEach(row => {
        const harga = parseFloat(row.querySelector(`${data1}`).value);
        const volume = parseFloat(row.querySelector(`${data2}`).value);
        const total = harga * volume;
        row.querySelector(`${element}`).textContent = total;
    })
}

const hargaTotalMinyak =() => hitungTotal('#tabelHargaMinyak tbody tr', '.hargaMinyak', '.volumeMinyak', '.total');
const hargaTotalInti =() => hitungTotal('#tabelHargaInti tbody tr', '.hargaInti', '.volumeInti', '.total');
