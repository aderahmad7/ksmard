						...
						<div class="table-responsive">
							<table class="table" id="table-grid" style="width: 100%;">
								<thead>
									<tr>
										<th width="1%">NO</th>
										<th width="29%">Uraian Penyusutan</th>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<h4>Laporan Biaya Penyusutan</h4>
					<ul class="list-group mb-3">
						
						<li class="list-group-item d-flex justify-content-between lh-condensed">
							<div>
								<h6 class="my-0">Total Biaya Penyusutan</h6>
								<small class="text-muted">Harga Rp dari jumlah seluruh biaya penyusutan</small>
							</div>
							<span class="text-muted" id="rekap-usut-total">0</span>
						</li>
						<li class="list-group-item d-flex justify-content-between lh-condensed">
							<div>
								<h6 class="my-0">Biaya per Kg</h6>
								<small class="text-muted">Harga Rp/Kg dari biaya penyusutan</small>
							</div>
							<span class="text-muted" id="rekap-usut-perkg">0</span>
						</li>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Input Biaya Penyusutan</h5>
				...
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="form-group col-lg-12 col-md-6 mb-3">
							<small class="fw-semibold">Uraian Penyusutan</small>
							<span class="help"></span>
							<div class="controls">
								<?php echo form_input('usutUraian', '', 'id="usutUraian" class="form-control"'); ?>
						...
						<div class="form-group col-lg-12 col-md-12 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_dropdown('usutKatusutKode', $kategoriPenyusutan, '', 'id="usutKatusutKode" class="nice-select default-select form-control wide"'); ?>
						...
						<div class="form-group col-lg-12 col-md-6 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_input('usutTotal', '', 'id="usutTotal" style="text-align:right;" class="form-control"'); ?>
	...
	<script>
		...
		$(document).ready(function() {
			oTable = $('#table-grid').DataTable({
				...
				pagingType: 'numbers',
				ajax: {
					url: '<?php echo site_url('pks/penyusutan/grid'); ?>',
					data: function(d) {
...

					{
						...
						render: function(data, type, row) {
							...
							if (row.indkStatus=="draft")
								...
								if (row.usutKomentar!=null){
									var komen = '<a data-id="' + data + '" style="margin :0px 0px 0px 0px ;" data-bs-backdrop="static" data-bs-toggle="modal" data-bs-target="#modal-komentar" onclick="return setModalKomentar($(this),\'' + row.usutKomentar + '\');" href="#" title="Komentar" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-comment"></i></a> ';
		...
		/* Dengan Rupiah */
		var harga = document.getElementById('usutTotal');
		...
		function setModalSalesReport() {
			...
			$.ajax({
				url: "<?php echo base_url('pks/penyusutan/rekap'); ?>",
				data: {
				...
				success: function(response) {
					if (response.rekap) {
						$("#rekap-usut-total").html(formatRibuan(response.data.rekap_usut_total));
						$("#rekap-usut-perkg").html(formatRibuan(response.data.rekap_usut_perkg));
		...
		function getPeriode() {
			...
			$.ajax({
				url: "<?php echo base_url('pks/penyusutan/periode'); ?>",
				data: {
					periode: $("#indkKode").val()
				},
				type: "POST",
		...
		function hapus() {
			var id = id_delete;
			$.ajax({
				url: "<?php echo base_url('pks/penyusutan/hapus'); ?>",
				data: {
					'id': id
				},
				type: "POST",
		...
		function edit(obj) {
			...
			$.ajax({
				url: "<?php echo base_url('pks/penyusutan/edit'); ?>",
				data: {
					usutKode: id
				},
				type: "POST",
				...
				success: function(response) {
					if (response.edit) {
						$('#usutKatusutKode [value="' + response.data.usutKatusutKode + '"]').prop("selected", true);
						$("#usutUraian").val(response.data.usutUraian);
						$("#usutTotal").val(formatRupiahV3(response.data.usutTotal));

						$("#kode").val(response.data.usutKode);
						$(".fa-spinner").hide();
		...
		function setModalSimpan() {
			...
			$("#modal-form .modal-body").show();
			$("#btn-simpan").show();
			$("#modal-form #modal-title").html("Biaya Penyusutan");
		}
		...
		$(document).delegate('.simpan', 'click', function(e) {
			...
			$("#kode").val('');
			$.ajax({
				url: "<?php echo site_url("pks/penyusutan/simpan"); ?>",
				data: data,
				type: "POST",
				dataType: "JSON",
				beforeSend: function() {
