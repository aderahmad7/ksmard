						...
						<div class="table-responsive">
							<table class="table" id="table-grid" style="width: 100%;">
								<thead>
									<tr>
										<th width="1%">NO</th>
										<th width="20%">Uraian Biaya Tidak Langsung</th>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<h4>Laporan Biaya Op. Tidak Langsung</h4>
					<ul class="list-group mb-3">
						
						<li class="list-group-item d-flex justify-content-between lh-condensed">
							<div>
								<h6 class="my-0">Total Biaya Op. Tidak Langsung</h6>
								<small class="text-muted">Harga Rp dari jumlah seluruh biaya Op. Tidak Langsung</small>
							</div>
							<span class="text-muted" id="rekap-biayatl-total">0</span>
						</li>
						<li class="list-group-item d-flex justify-content-between lh-condensed">
							<div>
								<h6 class="my-0">Biaya per Kg</h6>
								<small class="text-muted">Harga Rp/Kg dari biaya Op. Tidak Langsung</small>
							</div>
							<span class="text-muted" id="rekap-biayatl-perkg">0</span>
						</li>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Input Biaya Operasional Tidak Langsung</h5>
				...
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="form-group col-lg-12 col-md-6 mb-3">
							<small class="fw-semibold">Uraian Biaya Tidak Langsung</small>
							<span class="help"></span>
							<div class="controls">
								<?php echo form_input('biaytlUraian', '', 'id="biaytlUraian" class="form-control"'); ?>
						...
						<div class="form-group col-lg-12 col-md-12 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_dropdown('biaytlKatbiayKode', $kategoriBiayaTL, '', 'id="biaytlKatbiayKode" class="nice-select default-select form-control wide"'); ?>
							</div>
						</div>
						...
						<div class="form-group col-lg-12 col-md-6 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_input('biaytlVolume', '', 'id="biaytlVolume" style="text-align:right;" class="form-control"'); ?>
						...
						<div class="form-group col-lg-12 col-md-6 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_input('biaytlTotal', '', 'id="biaytlTotal" style="text-align:right;" class="form-control"'); ?>
	...
	<script>
		...
		$(document).ready(function() {
			oTable = $('#table-grid').DataTable({
				...
				pagingType: 'numbers',
				ajax: {
					url: '<?php echo site_url('pks/biayatidaklangsung/grid'); ?>',
					data: function(d) {
...

					{
						...
						render: function(data, type, row) {
							...
							if (row.indkStatus=="draft")
								...
								if (row.biaytlKomentar!=null){
									var komen = '<a data-id="' + data + '" style="margin :0px 0px 0px 0px ;" data-bs-backdrop="static" data-bs-toggle="modal" data-bs-target="#modal-komentar" onclick="return setModalKomentar($(this),\'' + row.biaytlKomentar + '\');" href="#" title="Komentar" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-comment"></i></a> ';
		...
		/* Dengan Rupiah */
		var harga = document.getElementById('biaytlTotal');
		harga.addEventListener('keyup', function(e) {
			harga.value = formatRibuan(this.value, 'Rp ');
		});
		var vol = document.getElementById('biaytlVolume');
		...
		function setModalSalesReport() {
			...
			$.ajax({
				url: "<?php echo base_url('pks/biayatidaklangsung/rekap'); ?>",
				data: {
				...
				success: function(response) {
					if (response.rekap) {
						$("#rekap-biayatl-total").html(formatRibuan(response.data.rekap_biayatl_total));
						$("#rekap-biayatl-perkg").html(formatRibuan(response.data.rekap_biayatl_perkg));
		...
		function getPeriode() {
			...
			$.ajax({
				url: "<?php echo base_url('pks/biayatidaklangsung/periode'); ?>",
				data: {
					periode: $("#indkKode").val()
				},
				type: "POST",
		...
		function hapus() {
			var id = id_delete;
			$.ajax({
				url: "<?php echo base_url('pks/biayatidaklangsung/hapus'); ?>",
				data: {
					'id': id
				},
				type: "POST",
		...
		function edit(obj) {
			...
			$.ajax({
				url: "<?php echo base_url('pks/biayatidaklangsung/edit'); ?>",
				data: {
					biaytlKode: id
				},
				type: "POST",
				...
				success: function(response) {
					if (response.edit) {
						$('#biaytlKatbiayKode [value="' + response.data.biaytlKatbiayKode + '"]').prop("selected", true);
						$("#biaytlUraian").val(response.data.biaytlUraian);
						$("#biaytlTotal").val(formatRupiahV3(response.data.biaytlTotal));
						$("#biaytlVolume").val(formatRibuan(response.data.biaytlVolume));

						$("#kode").val(response.data.biaytlKode);
						$(".fa-spinner").hide();
		...
		function setModalSimpan() {
			...
			$("#modal-form .modal-body").show();
			$("#btn-simpan").show();
			$("#modal-form #modal-title").html("Biaya Tidak Langsung");
		}
		...
		$(document).delegate('.simpan', 'click', function(e) {
			...
			$("#kode").val('');
			$.ajax({
				url: "<?php echo site_url("pks/biayatidaklangsung/simpan"); ?>",
				data: data,
				type: "POST",
				dataType: "JSON",
				beforeSend: function() {
