...
class Periode_m extends Model
{
    ...
    protected $allowedFields = ['olahKode', 'olahVolume', 'olahIndkKode'];

    public function getRekap($periode)
    {
        return $this->table($this->table)
            ->select('SUM(olahVolume) AS volume')
            ->join('ksmard_t_indeks_k_pks', 'ksmard_t_indeks_k_pks.indkKode = ksmard_t_produksi_diolah.olahIndkKode')
