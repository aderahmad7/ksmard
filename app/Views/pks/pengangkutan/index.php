						...
						<div class="table-responsive">
							<table class="table" id="table-grid" style="width: 100%;">
								<thead>
									<tr>
										<th width="1%">NO</th>
										<th width="20%">Uraian Pengangkutan</th>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Input Biaya Pengangkutan</h5>
				...
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="form-group col-lg-12 col-md-6 mb-3">
							<small class="fw-semibold">Uraian Pengangkutan</small>
							<span class="help"></span>
							<div class="controls">
								<?php echo form_input('angUraian', '', 'id="angUraian" class="form-control"'); ?>
						...
						<div class="form-group col-lg-6 col-md-6 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_dropdown('angIsEkspor', $katJual, '', 'id="angIsEkspor" class="nice-select default-select form-control wide"'); ?>
						...
						<div class="form-group col-lg-6 col-md-6 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_dropdown('angTbsKode', $katTbs, '', 'id="angTbsKode" class="nice-select default-select form-control wide"'); ?>
						...
						<div class="form-group col-lg-12 col-md-6 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_input('angVolume', '', 'id="angVolume" style="text-align:right;" class="form-control"'); ?>
						...
						<div class="form-group col-lg-12 col-md-6 mb-3">
							...
							<span class="help"></span>
							<div class="controls">
								<?php echo form_input('angTotal', '', 'id="angTotal" style="text-align:right;" class="form-control"'); ?>
	...
	<script>
		...
		$(document).ready(function() {
			oTable = $('#table-grid').DataTable({
				...
				pagingType: 'numbers',
				ajax: {
					url: '<?php echo site_url('pks/pengangkutan/grid'); ?>',
					data: function(d) {
...

					{
						...
						render: function(data, type, row) {
							...
							if (row.indkStatus=="draft")
								...
								if (row.angKomentar!=null){
									var komen = '<a data-id="' + data + '" style="margin :0px 0px 0px 0px ;" data-bs-backdrop="static" data-bs-toggle="modal" data-bs-target="#modal-komentar" onclick="return setModalKomentar($(this),\'' + row.angKomentar + '\');" href="#" title="Komentar" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-comment"></i></a> ';
		...
		/* Dengan Rupiah */
		var harga = document.getElementById('angTotal');
		harga.addEventListener('keyup', function(e) {
			harga.value = formatRibuan(this.value, 'Rp ');
		});
		var vol = document.getElementById('angVolume');
		...
		function setModalSalesReport() {
			...
			$.ajax({
				url: "<?php echo base_url('pks/pengangkutan/rekap'); ?>",
				data: {
		...
		function getPeriode() {
			...
			$.ajax({
				url: "<?php echo base_url('pks/pengangkutan/periode'); ?>",
				data: {
					periode: $("#indkKode").val()
				},
				type: "POST",
		...
		function hapus() {
			var id = id_delete;
			$.ajax({
				url: "<?php echo base_url('pks/pengangkutan/hapus'); ?>",
				data: {
					'id': id
				},
				type: "POST",
		...
		function edit(obj) {
			...
			$.ajax({
				url: "<?php echo base_url('pks/pengangkutan/edit'); ?>",
				data: {
					angKode: id
				},
				type: "POST",
				...
				success: function(response) {
					if (response.edit) {
						$('#angKatTbs [value="' + response.data.angKatTbs + '"]').prop("selected", true);
						$('#angIsEkspor [value="' + response.data.angIsEkspor + '"]').prop("selected", true);
						$("#angUraian").val(response.data.angUraian);
						$("#angTotal").val(formatRupiahV3(response.data.angTotal));
						$("#angVolume").val(formatRibuan(response.data.angVolume));

						$("#kode").val(response.data.angKode);
						$(".fa-spinner").hide();
		...
		function setModalSimpan() {
			...
			$("#modal-form .modal-body").show();
			$("#btn-simpan").show();
			$("#modal-form #modal-title").html("Biaya Pengangkutan");
		}
		...
		$(document).delegate('.simpan', 'click', function(e) {
			...
			$("#kode").val('');
			$.ajax({
				url: "<?php echo site_url("pks/pengangkutan/simpan"); ?>",
				data: data,
				type: "POST",
				dataType: "JSON",
				beforeSend: function() {
