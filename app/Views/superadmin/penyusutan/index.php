                        ...
                        <div class="table-responsive">
                            <table class="table" id="table-grid" style="width: 100%;">
                                <thead>
                                    <tr>
                                        <th width="1%">NO</th>
                                        <th width="90%">KATEGORI PENYUSUTAN</th>
                                        <th width="9%">AKSI</th>
                                    </tr>
                                </thead>
                            </table>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <input type="hidden" name="katusutKode" id="katusutKode">

                        <div class="form-group col-lg-12 col-md-12 mb-3">
                            <small class="fw-semibold">Nama Kategori</small>
                            <span class="help"></span>
                            <div class="controls">
                                <?php echo form_input('katusutNama', '', 'id="katusutNama" class="form-control"'); ?>
                            </div>
                        </div>
                    </div>
                    <?php echo form_close(); ?>
    ...
    <script>
        ...
        $(document).ready(function() {
            oTable = $('#table-grid').DataTable({
                ...
                pagingType: 'numbers',
                ajax: {
                    url: '<?php echo site_url('superadmin/penyusutan/grid'); ?>',
                },
                ...
                columns: [{
...

                    {
                        ...
                        render: function(data, type, row) {
                            console.log(row);
                            const katusutKode = row.katusutKode;

                            var edit = '<a data-id="' + katusutKode + '" style="margin :0px 1px 0px 0px ;" onclick="edit($(this));return false;" href="#" title="Ubah" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>';
                            var hapus = '<a data-id="' + katusutKode + '" style="margin :0px 0px 0px 0px ;" data-bs-backdrop="static" data-bs-toggle="modal" data-bs-target="#modal-hapus" onclick="return setModalHapus($(this),\'' + katusutKode + '\');" href="#" title="Hapus" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a> ';
                            return edit + hapus;
                        }
                    },
                ]
        ...
        function setModalSimpan() {
            ...
            $("#btn-simpan").show();
            $("#modal-form .modal-title").html("Tambah Kategori Penyusutan");
        }
        ...
        function edit(obj) {
            ...
            $.ajax({
                url: "<?php echo base_url('superadmin/penyusutan/edit'); ?>",
                data: {
                    katusutKode: id
                },
                type: "POST",
                ...
                beforeSend: function() {
                    ...
                    $(".box-msg").hide();
                    $("#modal-form .modal-title").html("Edit Kategori Penyusutan");
                    $(".fa-spinner").show();
                ...
                success: function(response) {
                    ...
                    if (response.edit) {
                        // Isi field sesuai data kategori penyusutan
                        $("#katusutKode").val(response.data.katusutKode);
                        $("#katusutNama").val(response.data.katusutNama);
        ...
        function setModalHapus(dom, x) {
            ...
            $("#modal-hapus .modal-header").html("Hapus Kategori Penyusutan");
            $("#modal-hapus .modal-body").html("Anda yakin menghapus kategori penyusutan \"<span id='id-delete'></span>\"?");
            $("#id-delete").html(x);
        }
        ...
        function hapus() {
            var id = id_delete;
            $.ajax({
                url: "<?php echo base_url('superadmin/penyusutan/hapus'); ?>",
                data: {
                    'id': id
                },
                type: "POST",
        ...
        $(document).delegate('.simpan', 'click', function(e) {
            ...
            $.ajax({
                url: "<?php echo site_url("superadmin/penyusutan/simpan"); ?>",
                data: data,
                type: "POST",
                dataType: "JSON",
                beforeSend: function() {
